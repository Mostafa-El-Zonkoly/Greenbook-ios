...
public class FTImageGridView: UIView {
    ...
    public func showWithImageArray(_ imageArray : [String] , tapBlock : @escaping ((_ buttonsArray: [UIButton] , _ buttonIndex : NSInteger) ->())) {
        ...
        if imageArray.count > 0 {
            ...
            let imgHeight : CGFloat = (frame.size.width - FTImageGridViewImageMargin * 1) / 2
            for i in 0 ... imageArray.count-1 {
                let x = CGFloat(i % 2) * (imgHeight + FTImageGridViewImageMargin)
                let y = CGFloat(i / 2) * (imgHeight + FTImageGridViewImageMargin)  + FTImageGridViewImageMargin
    ...
    public class func getHeightWithWidth(_ width: CGFloat, imgCount: Int) -> CGFloat{
        let imgHeight: CGFloat = (width - FTImageGridViewImageMargin * 1) / 2
        let photoAlbumHeight : CGFloat = imgHeight * CGFloat(ceilf(Float(imgCount) / 2)) + FTImageGridViewImageMargin * CGFloat(ceilf(Float(imgCount) / 2)-1)
